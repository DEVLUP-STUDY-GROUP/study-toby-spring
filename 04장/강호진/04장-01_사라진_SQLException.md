...
- 예외가 발생하면 catch 블록으로 잡아내는 것까지는 좋으나, 아무것도 하지 않고 별 문제 없는 것처럼 넘어가 버리는 건 위험
    - 원치않는 예외가 발생하는 것보다도 훨씬 나쁜 일
    - 프로그램 실행 중에 예외가 발생하였는데 그것 을 무시하고 계속 진행해버리기 때문
...
- 체크 예외(checked exception)이라고 불리는 명시적인 처리가 필요한 예외를 사용하고 다루는 방법
- 자바에서 throws를 통해 발생시킬 수 있는 예외는 크게 세가지가 존재
    ...
    - Exception과 체크 예외
        ...
        - 일반적으로 예외라고하면 Exception 클래스의 서브클래스 중, RuntimeException을 상속하지 않은 것
            - 체크 예외라고 생각
...
- 예외처리 방법 1: 예외 복구
    ...
    - 네트워크 접속이 원할하지 않아서 예외가 발생 시
        - 일정 시간 대기했다가 다시 접속을 시도해보는 방법을 사용해서 예외 상화응로부터 복구를 시도할 수 있음
        - 정해진 횟수만큼 재시도해서 실해했다면 예외 복구는 포기
    - 예외처리 코드를 강제하는 체크 예외들은 예외를 어떤 식으로든 복구할 가능성이 있는 경우 사용
        - API를 사용하는 개발자로 하여금 예외 상황이 발생할 수 있음을 인식하도록 도와주고 적절한 처리를 시도해보록 요구함
- 예외처리 방법 2: 예외 회피
    - 예외처리를 자신이 담당하지 않고 자신을 호출한 쪽으로 던지는 방법
    - throws 문을 선언해서 예외가 발생하면 알아서 던져지게 하거나, catch문으로 예외를 잡은 후에 로그를 남기고 다시 예외를 던짐
    ...
    - 예외를 회피하는 것은 예외를 보구하는 것처럼 의도가 분명해야 함
        - 콜백/템플릿처럼 긴말한 관계에 있는 다른 오브젝트에서 예외처리 책임을 분명하게 지게함
        - 자신을 사용하는 쪽에서 예외를 다루는 게최선의 방법이라는 분명한 확신이 있어야 함
- 예외처리 방법 3: 예외 전환
    - 예외 회피와 비슷하게 예외를 보구해서 정상적인 상태로는 만들 수 없기 때문에 예외를 메소드 밖으로 던짐
    ...
    - 예외 전환의 목적
        - 내부에서 발생한 예외를 그대로 던지는 것이 그 예외 상황에 대한 적절한 의미를 부여해주지 못하는 경우
            ...
            - 서비스 계층 오브젝트에서 SQLException의 원인을 해석해서 대응하는 것도 불하능하지 않음
            - 특정 기술의 정보를 해석하는 코드를 비즈니스 로직에 담은 서비스 계층에 두는 것 매우 어색
        ...
        - 예외를 처리하기 쉽고 단순하게 만들기 위해 포장하는 경우
            ...
            - 주로 예외처리를 강하게 체크 예외를 언체크 예외인 런타임 예외로 바꾸는 경우에 사용
            - 반대로 애플리케이션 로직상에서 예외조건이 발견되거나 예외상황이 발생할 수도 있음
                - API가 던지는 예외가 아니라 애플리케이션 코드에서 의도적으로 던지는 예외
                - 이 경우, 체크 예외를 사용하는 것이 적절
                    - 비지니스 적인 의미가 있는 예외는 이에 대한 적절한 대응이나 복구 작업이 필요
            - 일발적으로 체크 예외를 계속 trhows를 사용해 넘기는 것은 무의미함
...
- DAO에 존재하는 SQLException에 대해 고민
    ...
    - 시스템의 예외라면 애블리케이션 레벨에서 복구할 방법이 없음
    - 애플리케이션 코드의 버그나 다루지 않은 범위를 벗어난 값 때문에 발생한 예외도 복구할 방법이 없음
    - 예외처리 전략을 적용해야 함
    - 필요없는 기계적인 throws 선언이 등장하도록 방치하지 말고, 가능한 빨리 언체크/런테임 예외로 전환해줘야 함
